...
def validate_query(query, DB_PATH=DB_PATH):
    ...
    if ";" in query.strip().rstrip(";"):
        _debug("Query validation failed: Query cannot contain semicolons.")
        return False
    if any(kw in query.upper() for kw in ("DROP", "DELETE", "INSERT", "UPDATE")):
        _debug("Query validation failed: Query cannot contain DROP, DELETE, INSERT, or UPDATE.")
        return False
    ...
    try:
        ...
        _debug(f"Query validation failed: SQL parse error: {e}")
        return False
    ...
    for table_node in parsed.find_all(exp.Table):
        ...
        if tname not in schema:
            _debug(f"Query validation failed: Table '{tname}' does not exist.")
            return False
        alias_map[alias] = tname

    if not alias_map:
        _debug("Query validation failed: No tables found in query.")
        return False
    ...
    for col_node in parsed.find_all(exp.Column):
        ...
        if table_ref:
            if table_ref not in alias_map:
                _debug(f"Query validation failed: Unknown table reference '{table_ref}'.")
                return False
            real_table = alias_map[table_ref]
            if col_name not in schema[real_table]:
                _debug(f"Query validation failed: Column '{col_name}' not found in table '{real_table}'.")
                return False
        else:
            found = any(col_name in schema[alias_map[a]] for a in alias_map)
            if not found:
                _debug(f"Query validation failed: Column '{col_name}' not found in any referenced table.")
                return False
        ...
        for col_node in agg_node.find_all(exp.Column):
            ...
            if real_table and col_name in schema[real_table]:
                ...
                if base_type not in NUMERIC_TYPES:
                    _debug(f"Query validation failed: '{func_name.upper()}({col_name})' used on non-numeric type '{dtype}'.")
                    return False
        ...
        for agg_node in parsed.find_all(agg_class):
            for col_node in agg_node.find_all(exp.Column):
                ...
                if real_table and col_name in schema[real_table]:
                    ...
                    base_type = dtype.split("(")[0].strip()
                    if base_type not in NUMERIC_TYPES:
                        _debug(f"Query validation failed: '{agg_class.__name__.upper()}({col_name})' used on non-numeric type '{dtype}'.")
                        return False
        ...
        if condition is None:
            ...
            if using:
                for col_node in using if isinstance(using, list) else [using]:
                    ...
                    if len(found_in) < 2:
                        _debug(f"Query validation failed: JOIN USING column '{col_name}' must exist in at least two tables.")
                        return False
            continue
        ...
        for eq_node in condition.find_all(exp.EQ):
            ...
            for side in sides:
                if isinstance(side, exp.Column):
                    ...
                    if table_ref:
                        if table_ref not in alias_map:
                            _debug(f"Query validation failed: Unknown table '{table_ref}' in JOIN condition.")
                            return False
                        real_table = alias_map[table_ref]
                        if col_name not in schema[real_table]:
                            _debug(f"Query validation failed: Column '{col_name}' not in table '{real_table}' (JOIN condition).")
                            return False
            ...
            if len(resolved) == 2:
                ...
                if dtype1 != dtype2:
                    _debug(f"Query validation failed: JOIN type mismatch: '{t1}.{c1}' ({dtype1}) vs '{t2}.{c2}' ({dtype2}).")
                    return False

    _debug("Query validation passed.")
    return True